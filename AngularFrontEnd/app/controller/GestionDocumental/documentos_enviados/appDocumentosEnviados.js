app.controller('documentosEnviadosController', function ($scope, $timeout, DreamFactory,CONFIG,$window,$rootScope,sessionService,ngTableParams,$filter,$route, sweet, $http,FileUploader,$sce) {
  ...
  $scope.clase22='col-md-12';
    ...
    $scope.updFirmar = function(datos){
      console.log(datos);
      console.log( $scope.datosParafirma);
      // $scope.datadocumentos1[i].vdoct_titulo
      console.log($scope.datosParafirma.vdoc_idd);
        //$.blockUI();
        if($scope.resultado1){
        var consulta = {};
        ...
        resUsuario.identificador = 'RCCIUDADANO_91';
        var usu = sessionService.get('USUARIO');
        resUsuario.parametros='{"doc_modificacion":"'+ fechactual +'",doc_firma_digital":1,"doc_usuario":"'+ usu +'"}';
        consulta.llamarregla(function(respuesta){
...
      json = json + "]";
    ...
    var usu = sessionService.get('USUARIO');
    resElaboracion.parametros ='{"sistema":"'+ DMS +'",sproceso":"ELABORACION","somdnodo":"DMS","sdoc_datos":"'+ valores +'","susuario":"'+ usu +'","scuerpo":"'+ data.vdoc_cuerpo +'","sdoc_tiempo":"'+ data.vdoc_tiempo +'","sdoc_tps_doc_id":"'+ data.vtps_doc_id +'","sdoc_tipo_ingreso":"I","sdoc_acceso":"'+ data.vdoc_acceso +'"}';
    ...
    /********s5**********/
    // no hay sp_lista_fecha_version en la base de datos 
    /*****************/
  ...
  $scope.recuperalogelevaciones = function (idlog){ 
    ...
    /**************s6***********/
        // no hay sp_lista_cuerpo_version en la base de datos 
    /**********************/
...
      json = json + "]";     
    ...
    $scope.getDocumento = function(usuario,sistema,proceso,ci_nodo){        
        $.blockUI();
        var resRoles = {
            "procedure_name":"dms_sp_listar_doc_enviados",
             "body": {
              "params": [
                {
                  "name": "sdoc_usuario",
    ...
    /************s8******/
    //no hay dms_sp_listar_doc_enviados
    /********************/ 
    /**********s9*********/
    ...
    $scope.eliminarDocumento = function(vdoc_id){
        ...
        var USU = sessionService.get('IDUSUARIO');
        resRoles.parametros = '{"sdoc_idd":"'+ vdoc_id +'","sdoc_estado":"B","sdoc_usuario":"'+ USU +'"}';
        resRoles.llamarregla(function(response){
        response = JSON.parse(response);                  
            $.unblockUI();            
...
    json = json + "]";
    var valores = JSON.stringify(json);
    $.ajax({
      url:CONFIG.API_URL_DMS+'insertar8.php',
      type:"post",
