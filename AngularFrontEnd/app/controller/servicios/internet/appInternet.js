...
app.controller('internetController', function ($scope,$timeout, $rootScope, $routeParams, $location, $http, Data, sessionService,CONFIG, LogGuardarInfo, DreamFactory, $element, sweet, registroLog, filterFilter,FileUploader,ngTableParams, $filter, fileUpload,wsRgistrarPubliciadad, $window) {
    ...
    /*************s1******************/
     $scope.validarFormProcesos = function(datosForm){
        ...
        param.id = 57;
        param.parametros='{"ids":"'+ idTramite +'",frm_tra_enviado":"SI","frm_tra_if_codigo":"'+ nroTramiteEnviado +'","frm_tra_id_usuario":"'+ dUsuario +'","frm_tra_modificado":"'+ fechactual +'"}';
        param.llamarregla(function(results){
        results = JSON.parse(results);
            ...
            //sweet.show("Trámite creado", "Señor Ciudadano su Nro de trámite  es : " + nroTramiteEnviado); 
...
*/
    ...
    $scope.distritoZonas = function(idMacroJ){
        ...
        var sparam = new reglasnegocio();
        sparam.id = 26;
        sparam.parametros = '{"zona": "'+ idMacro +'"}';   
        sparam.llamarregla(function(results){
        results = JSON.parse(results);
            if(results.length > 0){
    ...
    $scope.vias= function(zona,tipo){
        ...
        var sparam = new reglasnegocio();
        sparam.id = 78;
        sparam.parametros='{"idz": "'+ zona +'","tipo": "'+ tipo +'"}';   
        sparam.llamarregla(function(results){
        results = JSON.parse(results);
            $scope.tip_vias =   [];
            var aTipoVia    =   {};
    ...
    $scope.recuperarInfo = function(){
        ...
        if(sIdCiudadano){
            ...
            datosCiudadano.llamarregla(function(results){
                results = JSON.parse(results);
                console.log("recuperandoooooooooo::",results);
                if(results.length > 0){
