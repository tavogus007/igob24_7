function regularjuridicoController($scope,$timeout, $rootScope, $routeParams, $location, $http,$q, Data, sessionService,CONFIG, LogGuardarInfo, $element, sweet, registroLog, filterFilter,FileUploader,ngTableParams, $filter, fileUpload,fileUpload1,wsRgistrarPubliciadad, $window, obtFechaActual) {
    var fecha           = new Date();
   ...
   $scope.obtenerHora = function(){
    ...
    var clsIniciarHtmlForm = $rootScope.$on('inicializarHtmlForm', function(event, tramite){
        ...
        if (tramite.venviado == 'NO' && JSON.stringify(datosgen) === '{}') {
         ...
         $.unblockUI();
        //$scope.initMap();
        $scope.open_mapa_ae_juridico();

    });
    /////////////////////////////////////////////
    $scope.open_mapa_ae_juridico = function()
    {
        setTimeout(function()
        {
            console.log("ENTRANDO AL MAPA AE JURIDICO...");
            var latitud = $scope.datos.INT_AC_latitud;
            var longitud = $scope.datos.INT_AC_longitud;
            console.log("latitud...",latitud);
            //map.removeLayer(vectorLayer_inci_baja);
            $("#mapActividadAEJuridico").empty();

            $scope.map = new ol.Map
            ({
              target: 'mapActividadAEJuridico',
              layers: [
                        new ol.layer.Group({
                                            title: 'Mapas Base',
                                            layers: [
