...
app.controller('cementerioGeoController', function ($scope, $rootScope, $http, servicios) {
    ...
    $scope.mapa = {
        ...
        acciones: {
            iniciar: function () {
                ...
                $scope.modoCel = getParameterByName('mCel');
                //console.log("modo celu",$scope.modoCel);

                //Mapa
                lays = new SITLayers();
                $scope.m_app = new SITMap();
                center = [590560.091, 8175936.283];
                tottemPing = [-68.151167,-16.495745];
                tottemPing2 = [-68.151167,-16.495754];
                tottem = [-68.151130,-16.495760];
                encuadre = [[590334.9,8176077.8],[590842.4,8175780.3]];
                var zoom = 11.2;
                ...
                //coord_msg=tottem;
                //coord_msg[1]=coord_msg[1]-$scope.offsetyTTM;
                //pcoord_msg = ol.proj.fromLonLat(coord_msg,ol.proj.get('EPSG:32719'));
                $scope.mapPopupContent.innerHTML = '<p>USTED ESTA AQUÍ</p>';
                
                // if($scope.modoCel =="1"){
                //     $scope.TottemArray.push(new ol.Feature({geometry: new ol.geom.Point(center)}));
                //     // pcoord_msg=center;
                //     // pcoord_msg[1]=pcoord_msg[1]-$scope.offsetyTTM;
            ...
            obtenerUbicacion: function () {
                ...
                $scope.pGPS = $scope.geolocation.getPosition();
                //console.log($scope.pGPS,encuadre);
                //console.log($scope.pGPS);
                var encIni=encuadre[0];
                var encFin=encuadre[1];
                //$scope.pGPS=[590560.091, 8175936.283]//quitar en prod
                //validar posicion inicial
                if($scope.pGPS!=null){
                    if($scope.pGPS[0]>encIni[0] && $scope.pGPS[0]<encFin[0]  && $scope.pGPS[1]<encIni[1] && $scope.pGPS[1]>encFin[1]){
                        return $scope.pGPS;    
                    }
                    else{
                        swal('Atención', 'Su ubicación no esta dentro del mapa del Cementerio General.', 'warning');
                        var mm = ol.proj.fromLonLat(tottemPing,ol.proj.get('EPSG:32719'));
                        return mm;
                    }
                }
                else{
                ...
                //     $scope.coordMovil=navigator.geolocation;
                //     //console.log($scope.coordMovil);

                //     //validar si esta en el poligono
                //     return $scope.coordMovil;
                // } else {
    ...
    $scope.servOSM=function (objCenter) {
        ...
        //if($scope.modoCel =="1"){
        $scope.pMovil = $scope.mapa.acciones.obtenerUbicacion();
        
        //ubicar punto 
        $scope.movil = ol.proj.toLonLat($scope.pMovil,ol.proj.get('EPSG:32719'));
        var urlx = "https://routing.openstreetmap.de/routed-foot/route/v1/driving/";
        urlx=urlx+$scope.movil[0]+','+$scope.movil[1]+';'+coorLongLat[0]+','+coorLongLat[1] //inicio y destico
        urlx=urlx+'?overview=false&geometries=polyline&steps=true&hints=ow4Xg14mO4MZAQAAcAAAAAAAAADcAwAAc-bJQYmXIUEAAAAAbPWxQsoAAABRAAAAAAAAAMgCAAAOAAAAtknw-41EBP-WSfD7t0QE_wAArw7235zN%3B-qzthS9rfI1bAQAAfQAAAAAAAAAAAAAAVMcKQsZlR0EAAAAAAAAAABUBAABkAAAAAAAAAAAAAAAOAAAAZHHw-8gXBP9kcfD7yBcE_wAADwP235zN';
        //}
        ...
        $http({
            ...
            pcoord1 = ol.proj.fromLonLat($scope.movil,ol.proj.get('EPSG:32719'));
            if($scope.modoCel =="1"){
                pcoord1 = $scope.pMovil;
            }
        ...
        $scope.contenido = [
            {
                table: {
                    headerRows: 1,
                    widths: [ '88%'], //widths: [ '96%'], tamaño regular
                    body: [
                        [ {
                            stack: 
                            [
