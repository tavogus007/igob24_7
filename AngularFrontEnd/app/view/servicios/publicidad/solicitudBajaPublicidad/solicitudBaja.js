function solicitudBajaController($scope,sessionService, CONFIG,ngTableParams, $filter,fileUpload1) {
    ...
    $scope.listarSolicitudes = function(){
        ...
        try {
            ...
            lstSolicitud.tipo_servicio  =   'PUB-B';
...
                },message: "Espere un momento por favor ..." }); 
    ...
    async function generarPdfBaja(dataBaja,nroTramite) {
        ...
        if(dataBaja.tipoPersona == 'JURIDICO') {
            ...
            numDocumento = dataBaja.documentoRepresentante;  
            numExpedido = dataBaja.expedidoRepresentante;
        }
        else {
            nombre = (dataBaja.nombres).trim() + ' ' + (dataBaja.primerApellido).trim()+' '+ (dataBaja.segundoApellido).trim();
            ...
            content: [
                ...
                dataBaja.tipoPersona === 'JURIDICO' ? [
                    { text: '\nDatos persona jurídica', fontSize: 11, bold: true },
                    {
                        columns: [
                            ...
                            { width: '*', text: dataBaja.nit, fontSize: 9 }
                        ]
                    },    
                    {
                        columns: [
                            { width: 150, text: 'Razón social:', fontSize: 9, bold: true },
                            { width: '*', text: dataBaja.razonSocial, fontSize: 9 }
                        ]
                    }
                ] : "",
                ...
                { 
                    columns: [
                        ...
                        { width: '*', text: dataBaja.correo, fontSize: 9 }
                    ]
                },
                { 
                    columns: [
                        { width: 150, text: 'Teléfono o Celular:', fontSize: 9, bold: true },
                        { width: '*', text: dataBaja.celular, fontSize: 9 }
                    ]
                },
                { text: '\nSolicito la baja de los siguiente elementos publicitarios', fontSize: 10, bold: true },
                dataBaja.tipoPublicidad === 'Movil' ? [
                    {
                        style: 'tabla',
                        table: {
                            headerRows: 1,
                ...
                dataBaja.tipoPublicidad === 'Coorporativa' ? [
                    {
                        style: 'tabla',
                        table: {
                            headerRows: 1,
