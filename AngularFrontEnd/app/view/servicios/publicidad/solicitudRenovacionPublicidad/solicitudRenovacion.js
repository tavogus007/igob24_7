function solicitudBajaController($scope,sessionService, CONFIG,ngTableParams, $filter) {
    ...
    $scope.tblPublicidadMovil =   {};
    $scope.publicidadMovil    =   [];
    $scope.codigoTramite = 'PUB-R';
    $scope.pubMovil = false;
    ...
    $scope.listarSolicitudes = function(){
        ...
        try {
            ...
            lstSolicitud.tipo_servicio  =   $scope.codigoTramite;
    ...
    $scope.buscarPublicidad = function(data){
        $.blockUI(); 
        $scope.publicidadMovil    =   [];
        $scope.pubMovil = false;
        $scope.publicidadCoorporativa    =   [];
        ...
        buscarPubli.buscarPublicidades(function(resultadoPub){
            ...
            if(respuesta.success){
                if(respuesta.success.dataSql.length > 0){
                    $.unblockUI();   
                    if(data.tipoPublicidad == 'Movil'){
                        $scope.publicidadMovil    =   respuesta.success.dataSql;
                        $scope.pubMovil = true;
                        $scope.tblPublicidadMovil.reload();
                    }else if(data.tipoPublicidad == 'Coorporativa'){
    ...
    $scope.tblPublicidadMovil = new ngTableParams({
        page: 1,
        count: 5,
        filter: {},
        sorting: {
        ...
        total: $scope.publicidadMovil.length,
        getData: function($defer, params) {
            var filteredData = params.filter() ?
            $filter('filter')($scope.publicidadMovil, params.filter()) :
            $scope.publicidadMovil;
            var orderedData = params.sorting() ?
            $filter('orderBy')(filteredData, params.orderBy()) :
            $scope.publicidadMovil;
            params.total($scope.publicidadMovil.length);
            $defer.resolve(orderedData.slice((params.page() - 1) * params.count(), params.page() * params.count()));
        }
    });
    ...
    $scope.guardarSolicitud =  function(data){
        ...
        if (data.publicidadesBaja.length>0) {       
            ...
            creaSolicitud.cod_servicio =  $scope.codigoTramite;
    ...
    $scope.generarPdfRenovacion = function(dataRenovacion,nroTramite) {
        ...
        var encabezado = [];
        var contenidoTabla = [];
        if(dataRenovacion.tipoPublicidad == 'Coorporativa'){
            encabezado = [
        ...
        }else if(dataRenovacion.tipoPublicidad == 'Movil'){
            encabezado = [
                { text: 'Nro.', style: 'tablaEncabezado' ,alignment: 'center'},  
                { text: 'ID Publicidad', style: 'tablaEncabezado' ,alignment: 'center'},  
                { text: 'Tipo letrero', style: 'tablaEncabezado',alignment: 'center' },
        ...
        if(dataRenovacion.tipoPersona == 'JURIDICO') {
            ...
            numDocumento = dataRenovacion.documentoRepresentante;  
            numExpedido = dataRenovacion.expedidoRepresentante;
        }
        else {
            nombre = (dataRenovacion.nombres).trim() + ' ' + (dataRenovacion.primerApellido).trim()+' '+ (dataRenovacion.segundoApellido).trim();
            numDocumento = dataRenovacion.documento;
            ...
            content: [
                ...
                dataRenovacion.tipoPersona === 'JURIDICO' ? [
                    { text: '\nDatos persona jurídica', fontSize: 11, bold: true },
                    {
                        columns: [
                            ...
                            { width: '*', text: dataRenovacion.nit, fontSize: 9 }
                        ]
                    },    
                    {
                        columns: [
                            { width: 150, text: 'Razón social:', fontSize: 9, bold: true },
                            { width: '*', text: dataRenovacion.razonSocial, fontSize: 9 }
                        ]
                    }
                ] : "",
                ...
                { 
                    columns: [
                        ...
                        { width: '*', text: dataRenovacion.correo, fontSize: 9 }
                    ]
                },
                { 
                    columns: [
                        { width: 150, text: 'Teléfono o Celular:', fontSize: 9, bold: true },
                        { width: '*', text: dataRenovacion.celular, fontSize: 9 }
                    ]
                },
                { text: '\nSolicito la renovación de los siguiente elementos publicitarios', fontSize: 10, bold: true },
                dataRenovacion.tipoPublicidad === 'Movil' ? [
                    {
                        style: 'tabla',
                        table: {
                            headerRows: 1,
                ...
                dataRenovacion.tipoPublicidad === 'Coorporativa' ? [
                    {
                        style: 'tabla',
                        table: {
                            headerRows: 1,
