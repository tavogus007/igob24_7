function solicitudPublicidadController($scope,sessionService, CONFIG,ngTableParams, $filter,$route,$http) {
...
/***********************************FIN CAPTCHA**********************/
  $scope.guardarSolicitud = function(datos){
      if ($scope.validarCamposLlenos(datos)) { 
      //$.blockUI();         
      datos.origen_solicitud = "igob";   
      datos.f01_publicidad = "EVENTUAL"; 
      datos.f01_listaeventual=$scope.inputs;
      var creaSolicitud = new adicionaRegistroSolicitud();
      creaSolicitud.cod_servicio = 'PUB-E';
      ...
      try {
          creaSolicitud.adicionaRegistroSolicitudServicio(function(res){
              ...
              if(respuesta.length  > 0){                  
                  ...
                  $scope.limpiaCampos();
                  $scope.generarPDFeventual(respuesta[0]);
...
// Rellenar los combos para cada input
$scope.inputs.forEach(function(input) {
  obtenerCategorias(input);
});
// Función para rellenar los combos de categorías
function obtenerCategorias(input) {
  $http.post(CONFIG.CONEXION_API_PG_IF+'wsGENESIS/buscarCategoriaPublicidad', { categoria: 'Eventual' })
    .then(function(response) {
     input.categorias = response.data.success.dataSql.slice(1);
    // $scope.$apply();
    })
...
$scope.cargarCaracteristicas = function(input) {
  var idTipoLetrero = input.categoria.idTipoLetrero;
  var data = { idTipoLetrero: idTipoLetrero };

  $http.post(CONFIG.CONEXION_API_PG_IF+'wsGENESIS/buscarCaracteristicaPublicidad', data)
    .then(function(response) {
      input.caracteristicas = response.data.success.dataSql.slice(1);
    })
    .catch(function(error) {
...
// Agregar un nuevo input al arreglo
$scope.agregarInput = function() {
  var nuevoInput = { categorias: [], caracteristicas: [] };
  $scope.inputs.push(nuevoInput);
  obtenerCategorias(nuevoInput);
};
// Eliminar un input del arreglo
$scope.eliminarInput = function(index) {
  $scope.inputs.splice(index, 1);
};
// Total superficie
$scope.subtotalsuperficie = function(input) { 
  var cantidadInput = input.cantidad;
  var superficieInput = input.superficie;
  var subtotal = cantidadInput * superficieInput;    
  input.subtotalsuperficie=subtotal.toFixed(2);
}; 
//*************************************  
$scope.generarPDFeventual = function(solicitud) {
  // Obtener los datos de la variable JSON
  var datosTabla = $scope.datos.f01_listaeventual;   
...
//**************************
$scope.validarCamposLlenos = function (datos) {
  if (!datos.f01_tipo_evento) {          
    swal("Por favor, completa todos los campos obligatorios (*) antes de continuar");  
    return false;
  }
  for (var i = 0; i < $scope.inputs.length; i++) {   
    var input = $scope.inputs[i];
    if (!input.categoria || !input.idTipoLetrero || !input.cantidad || !input.superficie || !input.direccion || !input.predio_municipal || !input.fecha_inicio || !input.fecha_fin) {          
        swal("Por favor, completa todos los campos obligatorios (*) antes de continuar");
      return false;
    }
  }
...
//**************************
$scope.validarFechas = function(input) {
  if (input.fecha_inicio && input.fecha_fin) {
      var fechaInicio = new Date(input.fecha_inicio);
      var fechaFin = new Date(input.fecha_fin);
      if (fechaInicio > fechaFin) {         
          swal("La fecha de conclusión debe ser mayor a la fecha de inicio.");
          input.fecha_inicio="";
          input.fecha_fin="";
          return false;
      }
