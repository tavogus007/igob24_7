function cierreViasController($scope, $rootScope, $routeParams, $location, $http, Data, sessionService,CONFIG, LogGuardarInfo, $element, sweet, ngTableParams, $filter, registroLog, filterFilter,FileUploader, fileUpload, $timeout, obtFechaCorrecta,$route, obtFechaActual,fileUpload1) {
    var sIdCiudadano= sessionService.get('IDSOLICITANTE');
    $scope.tipo_persona=sessionService.get('TIPO_PERSONA');
    $scope.oidCiu = sessionService.get('IDSOLICITANTE');
    $scope.tiposTramite = [
      { detalle: 'PERMISO EXCEPCIONAL DE CIERRE Y USO DE VIA PÚBLICA PARA LA EJECUCIÓN DE OBRAS CIVILES ', id:85 ,tipo:$scope.tipo_persona},
      { detalle: 'PERMISO EXCEPCIONAL DE CIERRE Y USO DE VÍA PÚBLICA PARA LA REALIZACION DE ACTIVIDADES DE EXPRESION CIUDADANA: CULTURALES, DEPORTIVAS, POLITICAS, CIVICAS, RELIGIOSAS Y SOCIALES:', id:86 ,tipo:$scope.tipo_persona},
      { detalle: 'PERMISO EXCEPCIONAL DE CIERRE Y USO DE VIA PÚBLICA PARA LA REALIZACIÓN DE ACTIVIDADES PREVISTAS EN EL CALENDARIO FESTIVO, FOLCLORICO Y RITUAL ', id:87 ,tipo:$scope.tipo_persona}
    ];
    $scope.datos = {};
    $scope.tablaTramites        =   {};
    ...
    [ { name: 'template0.html', url: '../../../app/index.html'},
      { name: 'template1.html', url: '../../../app/view/servicios/varios/formularios/movilidad/cierreVias/cierreV/cierreV.html'},
      //{ name: 'template2.html', url: '../../../app/view/servicios/varios/formularios/movilidad/denuncias/denunciaAlConductor/denunciaAlConductor.html'},
      //{ name: 'template2.html', url: '../../../app/view/servicios/varios/formularios/movilidad/denuncias/denunciaAlConductor/denunciaAlConductor.html'}
    ];
  
    $scope.template =   "";
  
    $scope.iniciovias = function(){
    ...
    $scope.tramitesCiudadano = function(){
      ...
      tramites.descripcion="MOVILIDAD";
    ...
    $scope.tramiteRender = function (tramite) {
      console.log("tramite");
      console.log(tramite);
      console.log(tramite.form_contenido);
      ...
      $scope.template = "";
      var idTemplate = 0;
      if($scope.procesoSeleccionado !=0){
        idTemplate = 1;
      }
      $scope.template = $scope.templates[idTemplate];
      setTimeout(function(){
        $rootScope.$broadcast('inicializarVista', tramite);
      },500);
      //$scope.$apply();
    };
  }
