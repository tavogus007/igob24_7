function registroOperadoresController($scope, $rootScope, $routeParams, $location, $http, Data, sessionService,CONFIG, LogGuardarInfo, $element, sweet, ngTableParams, $filter, registroLog, filterFilter,FileUploader, fileUpload, $timeout, obtFechaCorrecta,$route, obtFechaActual,fileUpload1) {
  ...
  $scope.validaEspacios = function(campo){
    var total = document.getElementById("RO_CAN_VEH_SUC").value;
    $scope.espaciosDisponibles = parseInt(total*0.10);
    numRegex = /^[0-9]+$/;
    if (numRegex.test(campo)) {
      if (campo < $scope.espaciosDisponibles){
        $scope.mensaje = true;
      }else{
        $scope.mensaje = false;
      }
  ...
  $scope.adiModOficinas = function(id,opc){
    ...
    if(swOfi == 0){
      ...
      && $scope.datos.RO_ENT_CALL_PM!='' && $scope.datos.RO_ENT_CALL_PM!=undefined){
        ...
        if($scope.datos.RO_ESP_DIS < $scope.espaciosDisponibles){
          swal("", "Los espacios reales disponibles en garaje deben ser mayores al 10% a la cantidad de vehículos disponibles", "warning"); 
        }else{
