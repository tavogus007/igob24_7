function solicitudPermisoCirculacionARVController($scope, $rootScope, $routeParams, $location, $http, Data, sessionService,CONFIG, LogGuardarInfo, $element, sweet, ngTableParams, $filter, registroLog, filterFilter,FileUploader, fileUpload, $timeout, obtFechaCorrecta,$route, obtFechaActual,fileUpload1) {
    var sIdCiudadano= sessionService.get('IDSOLICITANTE');
    $scope.tipo_persona=sessionService.get('TIPO_PERSONA');
    $scope.oidCiu = sessionService.get('IDSOLICITANTE');
    $scope.tiposTramite = [
        { detalle: 'Registro de Solicitud de PERMISO EXCEPCIONAL PARA CIRCULACIÓN EN EL ÁREA DE RESTRICCIÓN VEHICULAR', id:'41',tipo:$scope.tipo_persona}
    ];
    $scope.datos = {};
    $scope.tablaTramites        =   {};
    ...
    $scope.templates =
    [ 
      { name: 'template1.html', url: '../../../app/view/servicios/varios/formularios/movilidad/solicitudPermisoCirculacionARV/PermisoCirculacionARV/PermisoCirculacionARV.html'}
    ];
    ...
    $scope.tramitesCiudadano = function(){
      ...
      tramites.descripcion="PERMISO DE CIRCULACIÓN ARV";
    ...
    $scope.tramiteRender = function (tramite) {
      ...
      $scope.template = "";
      var idTemplate = 0;
      if($scope.procesoSeleccionado == $scope.deleveryID){
        idTemplate = 0;
      }  
    ...
    $scope.generarDocumentoPhp = function (){
        ...
        $.ajax({
            url:CONFIG.API_URL_DMS_2+'elaborarPdf/elaborar/elaborarDocPerMovilidad.php',
            type:"post",
            data:{
                "soid": oidCiudadano,
                "sorigen": "IGOB",
    ...
    $scope.InsertarDocumentoTv = function(urlData){
        ...
        var sDocProceso     =   "PERMISO DE CIRCULACIÓN ARV - DECLARACION JURADA";
        var sDocId          =   1;
        var sDocCiNodo      =   "CU";
        var sDocDatos       =   "";
        var sDocUrl         =   urlData;
        ...
        var sDocTipoDoc     =   "pdf";
        var sDocTamDoc      =   "";
        var sDocNombre      =   "PERMISO DE CIRCULACIÓN ARV - DECLARACION JURADA";
        var sDocTpsId       =   0;
