...
                },message: "Espere un momento porfavor..." }); 
    ...
    $scope.setSolicitudDatosPredio = function (data) {
        ...
        if (data.codigoMirador != null || data.codigoPlaza != null || data.cPresidencialDistancia != null || data.cPresidencialAltura != null || data.telefericoColor != null || $scope.solicitud.restriccionCH == 1 || data.instLegalPat != null || $scope.solicitud.restriccionSinCartilla == 1 || $scope.solicitud.restriccionPUC == 1)
        {
            ...
            if(resApi.success)
            {
                if(resApi.success.dataSql[0].d)
                {
                    ...
                    else 
                    {
                        $.unblockUI();
                        $scope.getSolicitudes();
                        swal('', 'Solicitud guardada correctamente', 'success');
                    }
                    $scope.flujo.paso = $scope.flujo.pasos.paso0;
                    //$scope.getSolicitudes();
                }
                ...
                {
                    $.unblockUI();
                    swal('', 'Error al guardar la solicitud', 'error');
